...
def usage():
    ...
    print('Don\'t support the Automation Framework')
    ...
    print('    --r_html          file to write the full ZAP HTML report')
    print('    --r_html_full     file to write the full ZAP HTML report with Request and Response')
    print('    --r_md            file to write the full ZAP Wiki (Markdown) report')
    print('    --r_xml           file to write the full ZAP XML report ')
    print('    --r_xml_full      file to write the full ZAP XML report with Request and Response')
    print('    --r_json          file to write the full ZAP JSON document')
    print('    --r_json_full     file to write the full ZAP JSON document with Request and Response')
...
def main(argv):
    ...
    try:
        opts, args = getopt.getopt(argv, "t:c:u:g:m:n:l:hdaijp:sz:P:D:T:IU:",
                                   ["hook=", "r_html=", "r_html_full=", "r_xml=", "r_xml_full=", "r_json=",
                                    "r_json_full=", "r_md="])
    ...
    for opt, arg in opts:
        if opt == '-h':
            ...
            report_html = arg
        elif opt == '--r_html_full':
            report_html_full = arg
        elif opt == '--r_xml':
            report_xml = arg
        elif opt == '--r_xml_full':
            report_xml_full = arg
            ...
            report_json = arg
        elif opt == '--r_json_full':
            report_json_full = arg
        elif opt == '--r_md':
            report_md = arg
    ...
    if running_in_docker():
        ...
        try:
            ...
            logging.warning('Failed to start ZAP in docker_1 :(')
            sys.exit(3)
    ...
    try:
        ...
        if num_urls == 0:
            ...
            if report_html:
                # Save the report
                zap.reports.generate(report_html, 'traditional-html', sites=target, reportdir=base_dir,
                                     reportfilename=report_html)

            if report_html_full:
                # Save the report
                zap.reports.generate(report_html_full, 'traditional-html-plus', sites=target, reportdir=base_dir,
                                     reportfilename=report_html_full)

            if report_json:
                # Save the report
                zap.reports.generate(report_json, 'traditional-json', sites=target, reportdir=base_dir,
                                     reportfilename=report_json)

            if report_json_full:
                # Save the report
                zap.reports.generate(report_json_full, 'traditional-json-plus', sites=target, reportdir=base_dir,
                                     reportfilename=report_json_full)

            if report_md:
                # Save the report
                zap.reports.generate(report_md, 'traditional-md', sites=target, reportdir=base_dir,
                                     reportfilename=report_md)

            if report_xml:
                # Save the report
                zap.reports.generate(report_xml, 'traditional-xml', sites=target, reportdir=base_dir,
                                     reportfilename=report_xml)

            if report_xml_full:
                # Save the report
                zap.reports.generate(report_xml_full, 'traditional-xml-plus', sites=target, reportdir=base_dir,
                                     reportfilename=report_xml_full)
