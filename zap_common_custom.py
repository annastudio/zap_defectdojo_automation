...
def load_config(config, config_dict, config_msg, out_of_scope_dict):
    ...
    for line in config:
        if line.startswith('#') or len(line.strip()) == 0:
            ...
            raise ValueError(
                "Unexpected number of tokens on line - there should be at least 3, tab separated: {0}".format(line))
        else:
...
def print_rules(zap, alert_dict, level, config_dict, config_msg, min_level, inc_rule, inc_extra, detailed_output,
                in_progress_issues):
    count = 0
    inprog_count = 0
...
def running_in_docker():
    return os.path.exists('/.dockerenv') or os.path.exists('/run/.containerenv') or os.environ.get(
...
def wait_for_zap_start(zap, timeout_in_secs=600):
    ...
    for x in range(0, timeout_in_secs):
        try:
            ...
            logging.info('ZAP Version ' + version)
            logging.debug('Took ' + str(x) + ' seconds')
            scan_policy = 'Default Policy'
            zap.ascan.update_scan_policy(scanpolicyname=scan_policy, alertthreshold="Medium", attackstrength="Medium")
            break
        except IOError:
            time.sleep(1)
...
def start_docker_zap(docker_image, port, extra_zap_params, mount_dir):
    ...
    logging.debug('Starting ZAP Docker container')
    params = ['docker', 'run']
...
def zap_spider(zap, target):
    if scan_user:
        logging.info('Spider %s as user %s', target, scan_user['name'])
        spider_scan_id = zap.spider.scan_as_user(context_id, scan_user['id'])
    else:
        logging.info('Spider %s', target)
    ...
    #     current_progress = int(zap.spider.status(spider_scan_id))
    #     if current_progress != previous_progress:
    #         logging.info('Spider progress %%: %s' % current_progress)
    #         previous_progress = copy(current_progress)
    #     time.sleep(5)
    # logging.info('Spider complete')

    while (int(zap.spider.status(spider_scan_id)) < 100):
        logging.info('Spider progress %: ' + zap.spider.status(spider_scan_id))
        time.sleep(5)
    logging.info('Spider complete')
...
def zap_active_scan(zap, target, policy):
    if scan_user:
        logging.info('Active Scan %s with policy %s as user %s', target, policy, scan_user['name'])
        ascan_scan_id = zap.ascan.scan_as_user(target, recurse=True, scanpolicyname=policy, contextid=context_id,
                                               userid=scan_user['id'])
    else:
        logging.info('Active Scan %s with policy %s', target, policy)
    ...
    current_progress = 0
    while current_progress < 100:
        current_progress = int(zap.ascan.status(ascan_scan_id))
        #progress_test = zap.ascan.scan_progress(ascan_scan_id)
        if current_progress != previous_progress:
            logging.info('Active Scan progress %%: %s' % current_progress)
            #logging.info('Active Scan progress %%: %s' % progress_test)
            previous_progress = copy(current_progress)
        time.sleep(5)
    logging.info('Active Scan complete')
    logging.info(zap.ascan.scan_progress(ascan_scan_id))


def zap_wait_for_passive_scan(zap, timeout_in_secs=0):
    logging.info('Zap pscan starting...')
    time_taken = 0
    timed_out = False
    while (int(zap.pscan.records_to_scan)) > 0:
        current_tasks = zap.pscan.current_tasks
        if current_tasks:
            logging.info('Currently running passive scan rule%%: %s' % current_tasks)
        ...
        if timeout_in_secs and time_taken > timeout_in_secs:
            ...
            break
    if timed_out:
        logging.info('Exceeded passive scan timeout')
    else:
